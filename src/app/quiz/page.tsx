...
export default function QuizPage() {
  ...
  const resultsContent = (
    <div style={{ padding: isDesktop ? '32px 40px 40px' : '60px 24px 40px', display: 'flex', flexDirection: 'column', maxWidth: 980, margin: '0 auto' }}>
  ...
  const quizContent = (
    <div style={{ padding: isDesktop ? '32px 40px' : '60px 24px 32px', display: 'flex', flexDirection: 'column', maxWidth: 520, margin: '0 auto', minHeight: isDesktop ? 'auto' : '100dvh' }}>
  ...
  const content = showResults ? resultsContent : quizContent;

  if (isDesktop) {
    return (
      <DesktopShell role="patient" activeId="search" title="Quiz de recomendación">
        {content}
      </DesktopShell>
    );
  }
