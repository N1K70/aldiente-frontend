  ...
  if (isDesktop) {
    return (
      <>
        ...
        {modalOpen && <UploadModal isStudent={isStudent} onClose={() => setModalOpen(false)} onUploaded={load} />}
      </>
    );
  }

  return (
    <div className="app-scroll" style={{ minHeight: '100dvh', overflowY: 'auto', background: 'var(--bg-aurora)', fontFamily: 'var(--font-body)', paddingBottom: 80 }}>
      ...
      <div style={{ padding: '0 20px' }}>{inner}</div>
      {modalOpen && <UploadModal isStudent={isStudent} onClose={() => setModalOpen(false)} onUploaded={load} />}
      <BottomNav />
    </div>
  );
